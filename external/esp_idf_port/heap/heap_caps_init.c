...
file.
*/
extern int _sbss, _sheap, _init_start, _iram_text_end;
...
void heap_caps_init()
{
    ...
    disable_mem_region(regions, (intptr_t)&_sbss, (intptr_t)&_sheap);           //DRAM used by bss/data static variables
    ...
    */
    ...
    for (int i = 0; i < soc_memory_region_count; i++) {
        ...
        pthread_mutex_init(&heap->heap_mux, NULL);
...
esp_err_t heap_caps_add_region_with_caps(const uint32_t caps[], intptr_t start, intptr_t end)
{
    ...
    p_new->start = start;
    p_new->end = end;
    pthread_mutex_init(&p_new->heap_mux, NULL);
    ...
    static pthread_mutex_t registered_heaps_write_lock=PTHREAD_MUTEX_INITIALIZER; 
    pthread_mutex_lock(&registered_heaps_write_lock);
    SLIST_INSERT_HEAD(&registered_heaps, p_new, next);
    pthread_mutex_unlock(&registered_heaps_write_lock);
