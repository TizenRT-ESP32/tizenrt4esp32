...
void scan_done_handler(void)
{
    ...
    if (esp_wifi_scan_get_ap_records(&sta_number, (wifi_ap_record_t *)ap_list_buffer) == ESP_OK) {
        for (i = 0; i < sta_number; i++) {
            ...
            snprintf(scan_list[i].ap_info.bssid, WIFI_UTILS_MACADDR_STR_LEN, "%02x:%02x:%02x:%02x:%02x:%02x",
                ap_list_buffer[i].bssid[0], ap_list_buffer[i].bssid[1], ap_list_buffer[i].bssid[2],
                ap_list_buffer[i].bssid[3], ap_list_buffer[i].bssid[4], ap_list_buffer[i].bssid[5] );
...
static esp_err_t event_handler(void *ctx, system_event_t *event)
{
    ...
    case SYSTEM_EVENT_STA_DISCONNECTED:
    {
        ...
        ESP_LOGI(TAG, "[%s] connect to the AP %s fail %d: %d; %d\n", __func__,
            event->event_info.disconnected.ssid, s_retry_num,
            event->event_info.disconnected.reason, isStaConnected);
