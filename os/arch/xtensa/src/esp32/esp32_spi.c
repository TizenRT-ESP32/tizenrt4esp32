...
#undef spiinfo
#if 0
#define spiinfo(format, ...)   do { printf(format, ##__VA_ARGS__); fflush(stdout); } while (0)
#else
...
static void spi_select(struct spi_dev_s *dev, enum spi_dev_e devid, bool selected)
{
	...
	case 0:
		...
		//priv->hw->pin.master_ck_sel |= 1 << 0;
		break;
	case 1:
		priv->hw->pin.cs1_dis = value;
		//priv->hw->pin.master_ck_sel |= 1 << 1;
		break;
	case 2:
		priv->hw->pin.cs2_dis = value;
		//priv->hw->pin.master_ck_sel |= 1 << 2;
		break;
